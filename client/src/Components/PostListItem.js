...
function PostListItem({
  ...
  console.log(editContent, editTag, editTitle);
  ...
  const DdaBongPlus = async () => {
    isOpen("true");
    await axios.post(`http://localhost:4000/post/${id}/plus`, {
      headers: { "Content-Type": "application/json" },
      Authorization: accessToken,
      withCredentials: true,
    });
  ...
  const DdaBongMinus = async () => {
    isOpen("false");
    await axios.post(`http://localhost:4000/post/${id}/minus`, {
      headers: { "Content-Type": "application/json" },
      Authorization: accessToken,
      withCredentials: true,
    });
  ...
  const handleEditPost = () => {
    ...
    axios
      .patch(
        `http://localhost:4000/post/${id}`, //id수정
        {
          title: editTitle,
          tag: editTag,
          content: editContent,
      ...
      .then((res) => {
        //navigate("/post/:id"); //게시물id를
      });
  };

  const handleDelComModal = (comId) => {
    console.log("open cdm");
    setComShow(!comShow);
    setCommentId(comId);
  };

  const handleDelNoCom = () => {
    console.log("no, not del com");
    setComShow(!comShow);
    navigate("/post");
  };
  ...
  const handleDelYesCom = async () => {
    console.log(id);
    setComShow(!comShow);
    await axios.post(`http://localhost:4000/comment/${id}/${commentId}`, {
      headers: { "Content-Type": "application/json" },
      Authorization: accessToken,
      withCredentials: true,
    });
